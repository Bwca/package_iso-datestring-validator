import { isValidTime } from '../src/iso-datestring-validator';
...
test(`isValidTime. expect 24:00 to validate and 24:01-59 to fail`, () => {
    for (let minute = 0; minute <= 59; minute++) {
        const validationResult = isValidTime(`24:${pad(minute.toString(), maxDigits)}`);
        minute === 0 ? expect(validationResult).toBe(true) : expect(validationResult).toBe(false);
    }
});
